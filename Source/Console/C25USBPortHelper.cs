...
namespace Devices
{
            ...
            if (!String.IsNullOrEmpty(instanceId))
            {
                try
                {
                    ...
                    foreach (var infoData in diData)
                    {
                        ...
                        {
                            if (id.StartsWith(instanceId.Substring(0, 21)))
                            {
                                // Disable port
    ...
    class USBDeviceInfo
    {
        public static List<USBDeviceInfo> C25GetFrontpanelUSBDevices()
        {
            ...
            {
                if (device.GetPropertyValue("PNPDeviceID").ToString().StartsWith("USB\\VID_2341&PID_8037"))
                {
